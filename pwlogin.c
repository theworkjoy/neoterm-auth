...
char *get_shell() {
    ...
    if (lstat(NEOTERM_HOME "/.neoterm/shell", &st) == 0) {
        if (S_ISLNK(st.st_mode)) {
            // If ~/.neoterm/shell points to executable.
            char link_target[PATH_MAX];
            ssize_t len = readlink(NEOTERM_HOME "/.neoterm/shell", link_target, sizeof(link_target) - 1);
            ...
            // If ~/.neoterm/shell is regular executable file.
            // Some NeoTerm users may prefer to launch a custom program
            // instead of shell.
            if (access(NEOTERM_HOME "/.neoterm/shell", R_OK | X_OK) == 0) {
                shell = NEOTERM_HOME "/.neoterm/shell";
            }
        } else {
            // If ~/.neoterm/shell has unexpected file type.
            fprintf(stderr, "Warning: ~/.neoterm/shell is not a regular file or symbolic link.\n");
        }
    } else {
...
// Setup the neoterm-exec hook if possible.
void prepare_neoterm_exec() {
    pid_t ch_pid, pid;
    int status;

    setenv("LD_PRELOAD", NEOTERM_PREFIX "/lib/libneoterm-exec.so", 1);

    ch_pid = fork();

    if (ch_pid == 0) {
        execl(NEOTERM_PREFIX "/bin/sh", "sh", "-c", "true", NULL);
    } else if (ch_pid > 0) {
        pid = wait(&status);
        if (WIFEXITED(status)) {
...
void init_login() {
    ...
    if (show_motd && (motd = fopen(NEOTERM_PREFIX "/etc/motd", "rb")) != NULL) {
        char *buf[256];
        int bytes_read;
...
int main(int argc, char **argv) {
    ...
    if (access(NEOTERM_HOME "/.hushlogin", F_OK) == 0 || getenv("NEOTERM_HUSHLOGIN") != NULL) {
        show_motd = false;
        unsetenv("NEOTERM_HUSHLOGIN");
    }

    for (int attempt=0; attempt<3; attempt++) {
        char *password = getpass("NeoTerm password: ");
        ...
        if (neoterm_auth("neoterm", password)) {
            init_login();
        } else {
